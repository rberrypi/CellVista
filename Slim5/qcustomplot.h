...
public:
	...
	// non-virtual methods:
	[[nodiscard]] double length() const { return qSqrt(mX * mX + mY * mY); }
	[[nodiscard]] double lengthSquared() const { return mX * mX + mY * mY; }
	[[nodiscard]] QPoint toPoint() const { return QPoint(mX, mY); }
	[[nodiscard]] QPointF toPointF() const { return QPointF(mX, mY); }

	[[nodiscard]] bool isNull() const { return qIsNull(mX) && qIsNull(mY); }
	void normalize();
	[[nodiscard]] QCPVector2D normalized() const;
	[[nodiscard]] QCPVector2D perpendicular() const { return QCPVector2D(-mY, mX); }
	[[nodiscard]] double dot(const QCPVector2D& vec) const { return mX * vec.mX + mY * vec.mY; }
	[[nodiscard]] double distanceSquaredToLine(const QCPVector2D& start, const QCPVector2D& end) const;
	[[nodiscard]] double distanceSquaredToLine(const QLineF& line) const;
	[[nodiscard]] double distanceToStraightLine(const QCPVector2D& base, const QCPVector2D& direction) const;
...
public:
	...
	*/
	...
	// getters:
	[[nodiscard]] bool antialiasing() const { return testRenderHint(QPainter::Antialiasing); }
	[[nodiscard]] PainterModes modes() const { return mModes; }
...
public:
	...
	void draw(QCPPainter* painter) const Q_DECL_OVERRIDE;
...
protected:
	...
	[[nodiscard]] virtual QCP::Interaction selectionCategory() const;
	[[nodiscard]] virtual QRect clipRect() const;
...
public:
	...
	[[nodiscard]] double size() const { return upper - lower; }
	[[nodiscard]] double center() const { return (upper + lower) * 0.5; }
...
public:
	...
	[[nodiscard]] QCPDataRange adjusted(int changeBegin, int changeEnd) const { return QCPDataRange(mBegin + changeBegin, mEnd + changeEnd); }
...
private:
	...
	static bool lessThanDataRangeBegin(const QCPDataRange& a, const QCPDataRange& b) { return a.begin() < b.begin(); }
};
...
public:
	...
	// getters:
	[[nodiscard]] QRect rect() const { return mRect; }
...
public:
	...
	// non-virtual methods:
	[[nodiscard]] QList<QCPLayoutElement*> elements(QCP::MarginSide side) const { return mChildren.value(side); }
...
protected:
	...
	// introduced virtual methods:
	[[nodiscard]] virtual int commonMargin(QCP::MarginSide side) const;
...
public:
	...
	*/
	...
	// getters:
	[[nodiscard]] QCPLayout* layout() const { return mParentLayout; }
	[[nodiscard]] QRect rect() const { return mRect; }
	[[nodiscard]] QRect outerRect() const { return mOuterRect; }
	[[nodiscard]] QMargins margins() const { return mMargins; }
	[[nodiscard]] QMargins minimumMargins() const { return mMinimumMargins; }
	[[nodiscard]] QCP::MarginSides autoMargins() const { return mAutoMargins; }
	[[nodiscard]] QSize minimumSize() const { return mMinimumSize; }
	[[nodiscard]] QSize maximumSize() const { return mMaximumSize; }
	[[nodiscard]] SizeConstraintRect sizeConstraintRect() const { return mSizeConstraintRect; }
	[[nodiscard]] QCPMarginGroup* marginGroup(QCP::MarginSide side) const { return mMarginGroups.value(side, (QCPMarginGroup*)0); }
	[[nodiscard]] QHash<QCP::MarginSide, QCPMarginGroup*> marginGroups() const { return mMarginGroups; }
	...
	[[nodiscard]] virtual QSize minimumOuterSizeHint() const;
	[[nodiscard]] virtual QSize maximumOuterSizeHint() const;
	[[nodiscard]] virtual QList<QCPLayoutElement*> elements(bool recursive) const;

	// reimplemented virtual methods:
	double selectTest(const QPointF& pos, bool onlySelectable, QVariant* details = 0) const Q_DECL_OVERRIDE;
...
protected:
	...
	void draw(QCPPainter* painter) Q_DECL_OVERRIDE { Q_UNUSED(painter) }
...
public:
	...
	[[nodiscard]] QList<QCPLayoutElement*> elements(bool recursive) const Q_DECL_OVERRIDE;

	// introduced virtual methods:
	[[nodiscard]] virtual int elementCount() const = 0;
	[[nodiscard]] virtual QCPLayoutElement* elementAt(int index) const = 0;
...
protected:
	...
	[[nodiscard]] QVector<int> getSectionSizes(QVector<int> maxSizes, QVector<int> minSizes, QVector<double> stretchFactors, int totalSize) const;
...
public:
	...
	*/
	...
	// getters:
	[[nodiscard]] int rowCount() const { return mElements.size(); }
	[[nodiscard]] int columnCount() const { return mElements.size() > 0 ? mElements.first().size() : 0; }
	[[nodiscard]] QList<double> columnStretchFactors() const { return mColumnStretchFactors; }
	[[nodiscard]] QList<double> rowStretchFactors() const { return mRowStretchFactors; }
	[[nodiscard]] int columnSpacing() const { return mColumnSpacing; }
	[[nodiscard]] int rowSpacing() const { return mRowSpacing; }
	[[nodiscard]] int wrap() const { return mWrap; }
	[[nodiscard]] FillOrder fillOrder() const { return mFillOrder; }
	...
	[[nodiscard]] int elementCount() const Q_DECL_OVERRIDE { return rowCount() * columnCount(); }
	[[nodiscard]] QCPLayoutElement* elementAt(int index) const Q_DECL_OVERRIDE;
	...
	[[nodiscard]] QSize minimumOuterSizeHint() const Q_DECL_OVERRIDE;
	[[nodiscard]] QSize maximumOuterSizeHint() const Q_DECL_OVERRIDE;

	// non-virtual methods:
	[[nodiscard]] QCPLayoutElement* element(int row, int column) const;
	...
	[[nodiscard]] int rowColToIndex(int row, int column) const;
...
public:
	...
	*/
	...
	// getters:
	[[nodiscard]] InsetPlacement insetPlacement(int index) const;
	[[nodiscard]] Qt::Alignment insetAlignment(int index) const;
	[[nodiscard]] QRectF insetRect(int index) const;
	...
	// reimplemented virtual methods:
	void updateLayout() Q_DECL_OVERRIDE;
	[[nodiscard]] int elementCount() const Q_DECL_OVERRIDE;
	[[nodiscard]] QCPLayoutElement* elementAt(int index) const Q_DECL_OVERRIDE;
	QCPLayoutElement* takeAt(int index) Q_DECL_OVERRIDE;
	bool take(QCPLayoutElement* element) Q_DECL_OVERRIDE;
	void simplify() Q_DECL_OVERRIDE {}
...
public:
	...
	*/
	...
	// non-property methods:
	[[nodiscard]] double boundingDistance() const;
	[[nodiscard]] double realLength() const;
...
protected:
	...
	[[nodiscard]] double pickClosest(double target, const QVector<double>& candidates) const;
	double getMantissa(double input, double* magnitude = 0) const;
	[[nodiscard]] double cleanMantissa(double input) const;
...
public:
	...
	// getters:
	[[nodiscard]] QString dateTimeFormat() const { return mDateTimeFormat; }
	[[nodiscard]] Qt::TimeSpec dateTimeSpec() const { return mDateTimeSpec; }
	...
	static double dateTimeToKey(QDateTime dateTime);
	static double dateTimeToKey(QDate date);
...
public:
	...
	*/
	...
	// getters:
	[[nodiscard]] QString timeFormat() const { return mTimeFormat; }
	[[nodiscard]] int fieldWidth(TimeUnit unit) const { return mFieldWidth.value(unit); }
...
public:
	...
	*/
	...
	// getters:
	[[nodiscard]] double tickStep() const { return mTickStep; }
	[[nodiscard]] ScaleStrategy scaleStrategy() const { return mScaleStrategy; }
...
public:
	...
	[[nodiscard]] int subTickCount() const { return mSubTickCount; }
...
public:
	...
	*/
	...
	// getters:
	[[nodiscard]] QString piSymbol() const { return mPiSymbol; }
	[[nodiscard]] double piValue() const { return mPiValue; }
	[[nodiscard]] bool periodicity() const { return mPeriodicity; }
	[[nodiscard]] FractionStyle fractionStyle() const { return mFractionStyle; }
...
protected:
	...
	[[nodiscard]] QString fractionToString(int numerator, int denominator) const;
	[[nodiscard]] QString unicodeFraction(int numerator, int denominator) const;
	[[nodiscard]] QString unicodeSuperscript(int number) const;
	[[nodiscard]] QString unicodeSubscript(int number) const;
};
...
public:
	...
	// getters:
	[[nodiscard]] double logBase() const { return mLogBase; }
	[[nodiscard]] int subTickCount() const { return mSubTickCount; }
...
public:
	...
	// getters:
	[[nodiscard]] bool subGridVisible() const { return mSubGridVisible; }
	[[nodiscard]] bool antialiasedSubGrid() const { return mAntialiasedSubGrid; }
	[[nodiscard]] bool antialiasedZeroLine() const { return mAntialiasedZeroLine; }
	[[nodiscard]] QPen pen() const { return mPen; }
	[[nodiscard]] QPen subGridPen() const { return mSubGridPen; }
	[[nodiscard]] QPen zeroLinePen() const { return mZeroLinePen; }
...
public:
	...
	*/
	...
	// getters:
	[[nodiscard]] AxisType axisType() const { return mAxisType; }
	[[nodiscard]] QCPAxisRect* axisRect() const { return mAxisRect; }
	[[nodiscard]] ScaleType scaleType() const { return mScaleType; }
	[[nodiscard]] const QCPRange range() const { return mRange; }
	[[nodiscard]] bool rangeReversed() const { return mRangeReversed; }
	[[nodiscard]] QSharedPointer<QCPAxisTicker> ticker() const { return mTicker; }
	[[nodiscard]] bool ticks() const { return mTicks; }
	[[nodiscard]] bool tickLabels() const { return mTickLabels; }
	[[nodiscard]] int tickLabelPadding() const;
	[[nodiscard]] QFont tickLabelFont() const { return mTickLabelFont; }
	[[nodiscard]] QColor tickLabelColor() const { return mTickLabelColor; }
	[[nodiscard]] double tickLabelRotation() const;
	[[nodiscard]] LabelSide tickLabelSide() const;
	[[nodiscard]] QString numberFormat() const;
	[[nodiscard]] int numberPrecision() const { return mNumberPrecision; }
	[[nodiscard]] QVector<double> tickVector() const { return mTickVector; }
	[[nodiscard]] QVector<QString> tickVectorLabels() const { return mTickVectorLabels; }
	[[nodiscard]] int tickLengthIn() const;
	[[nodiscard]] int tickLengthOut() const;
	[[nodiscard]] bool subTicks() const { return mSubTicks; }
	[[nodiscard]] int subTickLengthIn() const;
	[[nodiscard]] int subTickLengthOut() const;
	[[nodiscard]] QPen basePen() const { return mBasePen; }
	[[nodiscard]] QPen tickPen() const { return mTickPen; }
	[[nodiscard]] QPen subTickPen() const { return mSubTickPen; }
	[[nodiscard]] QFont labelFont() const { return mLabelFont; }
	[[nodiscard]] QColor labelColor() const { return mLabelColor; }
	[[nodiscard]] QString label() const { return mLabel; }
	[[nodiscard]] int labelPadding() const;
	[[nodiscard]] int padding() const { return mPadding; }
	[[nodiscard]] int offset() const;
	[[nodiscard]] SelectableParts selectedParts() const { return mSelectedParts; }
	[[nodiscard]] SelectableParts selectableParts() const { return mSelectableParts; }
	[[nodiscard]] QFont selectedTickLabelFont() const { return mSelectedTickLabelFont; }
	[[nodiscard]] QFont selectedLabelFont() const { return mSelectedLabelFont; }
	[[nodiscard]] QColor selectedTickLabelColor() const { return mSelectedTickLabelColor; }
	[[nodiscard]] QColor selectedLabelColor() const { return mSelectedLabelColor; }
	[[nodiscard]] QPen selectedBasePen() const { return mSelectedBasePen; }
	[[nodiscard]] QPen selectedTickPen() const { return mSelectedTickPen; }
	[[nodiscard]] QPen selectedSubTickPen() const { return mSelectedSubTickPen; }
	[[nodiscard]] QCPLineEnding lowerEnding() const;
	[[nodiscard]] QCPLineEnding upperEnding() const;
	[[nodiscard]] QCPGrid* grid() const { return mGrid; }
	...
	// non-property methods:
	[[nodiscard]] Qt::Orientation orientation() const { return mOrientation; }
	[[nodiscard]] int pixelOrientation() const { return rangeReversed() != (orientation() == Qt::Vertical) ? -1 : 1; }
	void moveRange(double diff);
	void scaleRange(double factor);
	void scaleRange(double factor, double center);
	void setScaleRatio(const QCPAxis* otherAxis, double ratio = 1.0);
	void rescale(bool onlyVisiblePlottables = false);
	[[nodiscard]] double pixelToCoord(double value) const;
	[[nodiscard]] double coordToPixel(double value) const;
	[[nodiscard]] SelectablePart getPartAt(const QPointF& pos) const;
	[[nodiscard]] QList<QCPAbstractPlottable*> plottables() const;
	[[nodiscard]] QList<QCPGraph*> graphs() const;
	[[nodiscard]] QList<QCPAbstractItem*> items() const;
...
protected:
	...
	void applyDefaultAntialiasingHint(QCPPainter* painter) const Q_DECL_OVERRIDE;
	void draw(QCPPainter* painter) Q_DECL_OVERRIDE;
	[[nodiscard]] QCP::Interaction selectionCategory() const Q_DECL_OVERRIDE;
	// events:
	void selectEvent(QMouseEvent* event, bool additive, const QVariant& details, bool* selectionStateChanged) Q_DECL_OVERRIDE;
	void deselectEvent(bool* selectionStateChanged) Q_DECL_OVERRIDE;
	// mouse events:
	...
	[[nodiscard]] QPen getBasePen() const;
	[[nodiscard]] QPen getTickPen() const;
	[[nodiscard]] QPen getSubTickPen() const;
	[[nodiscard]] QFont getTickLabelFont() const;
	[[nodiscard]] QFont getLabelFont() const;
	[[nodiscard]] QColor getTickLabelColor() const;
	[[nodiscard]] QColor getLabelColor() const;
...
public:
	...
	[[nodiscard]] QRect axisSelectionBox() const { return mAxisSelectionBox; }
	[[nodiscard]] QRect tickLabelsSelectionBox() const { return mTickLabelsSelectionBox; }
	[[nodiscard]] QRect labelSelectionBox() const { return mLabelSelectionBox; }
...
protected:
	...
	[[nodiscard]] virtual QByteArray generateLabelParameterHash() const;

	virtual void placeTickLabel(QCPPainter* painter, double position, int distanceToAxis, const QString& text, QSize* tickLabelsSize);
	virtual void drawTickLabel(QCPPainter* painter, double x, double y, const TickLabelData& labelData) const;
	[[nodiscard]] virtual TickLabelData getTickLabelData(const QFont& font, const QString& text) const;
	[[nodiscard]] virtual QPointF getTickLabelDrawOffset(const TickLabelData& labelData) const;
...
bool qcpLessThanSortKey(const DataType& a, const DataType& b) { return a.sortKey() < b.sortKey(); }
...
public:
	...
	[[nodiscard]] const_iterator at(int index) const { return constBegin() + qBound(0, index, size()); }
	QCPRange keyRange(bool& foundRange, QCP::SignDomain signDomain = QCP::sdBoth);
	QCPRange valueRange(bool& foundRange, QCP::SignDomain signDomain = QCP::sdBoth, const QCPRange& inKeyRange = QCPRange());
	[[nodiscard]] QCPDataRange dataRange() const { return QCPDataRange(0, size()); }
...
public:
	...
	[[nodiscard]] QCPScatterStyle::ScatterProperties usedScatterProperties() const { return mUsedScatterProperties; }
	...
	[[nodiscard]] QCPScatterStyle getFinalScatterStyle(const QCPScatterStyle& unselectedStyle) const;
...
public:
	...
	// introduced virtual methods:
	double selectTest(const QPointF& pos, bool onlySelectable, QVariant* details = 0) const Q_DECL_OVERRIDE = 0; // actually introduced in QCPLayerable as non-pure, but we want to force reimplementation for plottables
	...
	[[nodiscard]] const QPointF coordsToPixels(double key, double value) const;
	...
	[[nodiscard]] bool removeFromLegend() const;
...
protected:
	...
	// reimplemented virtual methods:
	[[nodiscard]] QRect clipRect() const Q_DECL_OVERRIDE;
	void draw(QCPPainter* painter) Q_DECL_OVERRIDE = 0;
	[[nodiscard]] QCP::Interaction selectionCategory() const Q_DECL_OVERRIDE;
	void applyDefaultAntialiasingHint(QCPPainter* painter) const Q_DECL_OVERRIDE;
	// events:
	void selectEvent(QMouseEvent* event, bool additive, const QVariant& details, bool* selectionStateChanged) Q_DECL_OVERRIDE;
...
public:
	...
	*/
	...
	[[nodiscard]] QPointF pixelPosition() const Q_DECL_OVERRIDE;
...
protected:
	...
	// reimplemented virtual methods:
	QCPItemPosition* toQCPItemPosition() Q_DECL_OVERRIDE { return this; }
...
public:
	...
	// reimplemented virtual methods:
	double selectTest(const QPointF& pos, bool onlySelectable, QVariant* details = 0) const Q_DECL_OVERRIDE = 0;
...
protected:
	...
	// introduced virtual methods:
	[[nodiscard]] virtual QPointF anchorPixelPosition(int anchorId) const;

	// non-virtual methods:
	[[nodiscard]] double rectDistance(const QRectF& rect, const QPointF& pos, bool filledRect) const;
...
public:
	...
	*/
	...
	[[nodiscard]] bool openGl() const { return mOpenGl; }
	...
	[[nodiscard]] QCPAbstractPlottable* plottableAt(const QPointF& pos, bool onlySelectable = false) const;
...
protected:
	...
	// reimplemented virtual methods:
	[[nodiscard]] QSize minimumSizeHint() const Q_DECL_OVERRIDE;
...
public:
	...
	[[nodiscard]] virtual QCPDataSelection selectTestRect(const QRectF& rect, bool onlySelectable) const = 0;
...
public:
	...
	[[nodiscard]] int dataCount() const Q_DECL_OVERRIDE;
	[[nodiscard]] double dataMainKey(int index) const Q_DECL_OVERRIDE;
	[[nodiscard]] double dataSortKey(int index) const Q_DECL_OVERRIDE;
	[[nodiscard]] double dataMainValue(int index) const Q_DECL_OVERRIDE;
	[[nodiscard]] QCPRange dataValueRange(int index) const Q_DECL_OVERRIDE;
	[[nodiscard]] QPointF dataPixelPosition(int index) const Q_DECL_OVERRIDE;
	[[nodiscard]] bool sortKeyIsMainKey() const Q_DECL_OVERRIDE;
	[[nodiscard]] QCPDataSelection selectTestRect(const QRectF& rect, bool onlySelectable) const Q_DECL_OVERRIDE;
	[[nodiscard]] int findBegin(double sortKey, bool expandedRange = true) const Q_DECL_OVERRIDE;
	[[nodiscard]] int findEnd(double sortKey, bool expandedRange = true) const Q_DECL_OVERRIDE;

	// reimplemented virtual methods:
	double selectTest(const QPointF& pos, bool onlySelectable, QVariant* details = 0) const Q_DECL_OVERRIDE;
	QCPPlottableInterface1D* interface1D() Q_DECL_OVERRIDE { return this; }
...
protected:
	...
	// non-virtual methods:
	[[nodiscard]] bool stopsUseAlpha() const;
...
public:
	...
	*/
	...
	// getters:
	[[nodiscard]] QPen bracketPen() const { return mBracketPen; }
	[[nodiscard]] QBrush bracketBrush() const { return mBracketBrush; }
	[[nodiscard]] int bracketWidth() const { return mBracketWidth; }
	[[nodiscard]] int bracketHeight() const { return mBracketHeight; }
	[[nodiscard]] BracketStyle bracketStyle() const { return mBracketStyle; }
	[[nodiscard]] bool tangentToData() const { return mTangentToData; }
	[[nodiscard]] int tangentAverage() const { return mTangentAverage; }
	...
	// virtual methods:
	void drawDecoration(QCPPainter* painter, QCPDataSelection selection) Q_DECL_OVERRIDE;
...
public:
	...
	[[nodiscard]] bool backgroundScaled() const { return mBackgroundScaled; }
	[[nodiscard]] Qt::AspectRatioMode backgroundScaledMode() const { return mBackgroundScaledMode; }
	[[nodiscard]] Qt::Orientations rangeDrag() const { return mRangeDrag; }
	[[nodiscard]] Qt::Orientations rangeZoom() const { return mRangeZoom; }
	...
	// non-property methods:
	[[nodiscard]] int axisCount(QCPAxis::AxisType type) const;
	[[nodiscard]] QCPAxis* axis(QCPAxis::AxisType type, int index = 0) const;
	[[nodiscard]] QList<QCPAxis*> axes(QCPAxis::AxisTypes types) const;
	[[nodiscard]] QList<QCPAxis*> axes() const;
	QCPAxis* addAxis(QCPAxis::AxisType type, QCPAxis* axis = 0);
	QList<QCPAxis*> addAxes(QCPAxis::AxisTypes types);
	bool removeAxis(QCPAxis* axis);
	[[nodiscard]] QCPLayoutInset* insetLayout() const { return mInsetLayout; }
	...
	[[nodiscard]] int left() const { return mRect.left(); }
	[[nodiscard]] int right() const { return mRect.right(); }
	[[nodiscard]] int top() const { return mRect.top(); }
	[[nodiscard]] int bottom() const { return mRect.bottom(); }
	[[nodiscard]] int width() const { return mRect.width(); }
	[[nodiscard]] int height() const { return mRect.height(); }
	[[nodiscard]] QSize size() const { return mRect.size(); }
	[[nodiscard]] QPoint topLeft() const { return mRect.topLeft(); }
	[[nodiscard]] QPoint topRight() const { return mRect.topRight(); }
	[[nodiscard]] QPoint bottomLeft() const { return mRect.bottomLeft(); }
	[[nodiscard]] QPoint bottomRight() const { return mRect.bottomRight(); }
	[[nodiscard]] QPoint center() const { return mRect.center(); }
...
protected:
	...
	void applyDefaultAntialiasingHint(QCPPainter* painter) const Q_DECL_OVERRIDE;
	void draw(QCPPainter* painter) Q_DECL_OVERRIDE;
	int calculateAutoMargin(QCP::MarginSide side) Q_DECL_OVERRIDE;
...
public:
	...
	// getters:
	[[nodiscard]] QCPLegend* parentLegend() const { return mParentLegend; }
	[[nodiscard]] QFont font() const { return mFont; }
	[[nodiscard]] QColor textColor() const { return mTextColor; }
	[[nodiscard]] QFont selectedFont() const { return mSelectedFont; }
	[[nodiscard]] QColor selectedTextColor() const { return mSelectedTextColor; }
	[[nodiscard]] bool selectable() const { return mSelectable; }
	[[nodiscard]] bool selected() const { return mSelected; }
...
protected:
	...
	// non-virtual methods:
	[[nodiscard]] QPen getIconBorderPen() const;
...
public:
	...
	*/
	...
	// getters:
	[[nodiscard]] QPen borderPen() const { return mBorderPen; }
	[[nodiscard]] QBrush brush() const { return mBrush; }
	[[nodiscard]] QFont font() const { return mFont; }
	[[nodiscard]] QColor textColor() const { return mTextColor; }
	[[nodiscard]] QSize iconSize() const { return mIconSize; }
	[[nodiscard]] int iconTextPadding() const { return mIconTextPadding; }
	[[nodiscard]] QPen iconBorderPen() const { return mIconBorderPen; }
	[[nodiscard]] SelectableParts selectableParts() const { return mSelectableParts; }
	[[nodiscard]] SelectableParts selectedParts() const;
	[[nodiscard]] QPen selectedBorderPen() const { return mSelectedBorderPen; }
	[[nodiscard]] QPen selectedIconBorderPen() const { return mSelectedIconBorderPen; }
	[[nodiscard]] QBrush selectedBrush() const { return mSelectedBrush; }
	[[nodiscard]] QFont selectedFont() const { return mSelectedFont; }
	[[nodiscard]] QColor selectedTextColor() const { return mSelectedTextColor; }
	...
	// non-virtual methods:
	[[nodiscard]] QCPAbstractLegendItem* item(int index) const;
	...
	void clearItems();
	[[nodiscard]] QList<QCPAbstractLegendItem*> selectedItems() const;
...
protected:
	...
	// non-virtual methods:
	[[nodiscard]] QPen getBorderPen() const;
	[[nodiscard]] QBrush getBrush() const;
...
public:
	...
	[[nodiscard]] int textFlags() const { return mTextFlags; }
	[[nodiscard]] QFont font() const { return mFont; }
	[[nodiscard]] QColor textColor() const { return mTextColor; }
	[[nodiscard]] QFont selectedFont() const { return mSelectedFont; }
	[[nodiscard]] QColor selectedTextColor() const { return mSelectedTextColor; }
...
protected:
	...
	// non-virtual methods:
	[[nodiscard]] QFont mainFont() const;
	[[nodiscard]] QColor mainTextColor() const;
...
public:
	...
	// getters:
	[[nodiscard]] QCPAxis* axis() const { return mColorAxis.data(); }
	[[nodiscard]] QCPAxis::AxisType type() const { return mType; }
	...
	[[nodiscard]] int barWidth() const { return mBarWidth; }
	[[nodiscard]] bool rangeDrag() const;
	[[nodiscard]] bool rangeZoom() const;
	...
	// non-property methods:
	[[nodiscard]] QList<QCPColorMap*> colorMaps() const;
...
public:
	...
	[[nodiscard]] double sortKey() const { return key; }
	static QCPGraphData fromSortKey(double sortKey) { return QCPGraphData(sortKey, 0); }
	static bool sortKeyIsMainKey() { return true; }

	[[nodiscard]] double mainKey() const { return key; }
	[[nodiscard]] double mainValue() const { return value; }

	[[nodiscard]] QCPRange valueRange() const { return QCPRange(value, value); }
...
public:
	...
	*/
	...
	// reimplemented virtual methods:
	double selectTest(const QPointF& pos, bool onlySelectable, QVariant* details = 0) const Q_DECL_OVERRIDE;
	QCPRange getKeyRange(bool& foundRange, QCP::SignDomain inSignDomain = QCP::sdBoth) const Q_DECL_OVERRIDE;
	QCPRange getValueRange(bool& foundRange, QCP::SignDomain inSignDomain = QCP::sdBoth, const QCPRange& inKeyRange = QCPRange()) const Q_DECL_OVERRIDE;
...
protected:
	...
	[[nodiscard]] QVector<QPointF> dataToLines(const QVector<QCPGraphData>& data) const;
	[[nodiscard]] QVector<QPointF> dataToStepLeftLines(const QVector<QCPGraphData>& data) const;
	[[nodiscard]] QVector<QPointF> dataToStepRightLines(const QVector<QCPGraphData>& data) const;
	[[nodiscard]] QVector<QPointF> dataToStepCenterLines(const QVector<QCPGraphData>& data) const;
	[[nodiscard]] QVector<QPointF> dataToImpulseLines(const QVector<QCPGraphData>& data) const;
	...
	[[nodiscard]] QPointF getFillBasePoint(QPointF matchingDataPoint) const;
...
public:
	...
	[[nodiscard]] double sortKey() const { return t; }
	static QCPCurveData fromSortKey(double sortKey) { return QCPCurveData(sortKey, 0, 0); }
	static bool sortKeyIsMainKey() { return false; }
...
protected:
	...
	[[nodiscard]] int getRegion(double key, double value, double keyMin, double valueMax, double keyMax, double valueMin) const;
	[[nodiscard]] QPointF getOptimizedPoint(int prevRegion, double prevKey, double prevValue, double key, double value, double keyMin, double valueMax, double keyMax, double valueMin) const;
	[[nodiscard]] QVector<QPointF> getOptimizedCornerPoints(int prevRegion, int currentRegion, double prevKey, double prevValue, double key, double value, double keyMin, double valueMax, double keyMax, double valueMin) const;
	[[nodiscard]] bool mayTraverse(int prevRegion, int currentRegion) const;
...
public:
	...
	[[nodiscard]] double sortKey() const { return key; }
	static QCPBarsData fromSortKey(double sortKey) { return QCPBarsData(sortKey, 0); }
	static bool sortKeyIsMainKey() { return true; }

	[[nodiscard]] double mainKey() const { return key; }
	[[nodiscard]] double mainValue() const { return value; }

	[[nodiscard]] QCPRange valueRange() const { return QCPRange(value, value); } // note that bar base value isn't held in each QCPBarsData and thus can't/shouldn't be returned here
...
protected:
	...
	[[nodiscard]] QRectF getBarRect(double key, double value) const;
	void getPixelWidth(double key, double& lower, double& upper) const;
	[[nodiscard]] double getStackedBaseValue(double key, bool positive) const;
...
public:
	...
	[[nodiscard]] double sortKey() const { return key; }
	static QCPStatisticalBoxData fromSortKey(double sortKey) { return QCPStatisticalBoxData(sortKey, 0, 0, 0, 0, 0); }
	static bool sortKeyIsMainKey() { return true; }

	[[nodiscard]] double mainKey() const { return key; }
	[[nodiscard]] double mainValue() const { return median; }

	[[nodiscard]] QCPRange valueRange() const
	{
...
public:
	...
	// getters:
	[[nodiscard]] int keySize() const { return mKeySize; }
	[[nodiscard]] int valueSize() const { return mValueSize; }
	[[nodiscard]] QCPRange keyRange() const { return mKeyRange; }
	[[nodiscard]] QCPRange valueRange() const { return mValueRange; }
	[[nodiscard]] QCPRange dataBounds() const { return mDataBounds; }
	...
	[[nodiscard]] bool isEmpty() const { return mIsEmpty; }
...
public:
	...
	[[nodiscard]] double sortKey() const { return key; }
	static QCPFinancialData fromSortKey(double sortKey) { return QCPFinancialData(sortKey, 0, 0, 0, 0); }
	static bool sortKeyIsMainKey() { return true; }

	[[nodiscard]] double mainKey() const { return key; }
	[[nodiscard]] double mainValue() const { return open; }

	[[nodiscard]] QCPRange valueRange() const { return QCPRange(low, high); } // open and close must lie between low and high, so we don't need to check them
...
protected:
	...
	[[nodiscard]] double getPixelWidth(double key, double keyPixel) const;
...
protected:
	...
	// helpers:
	void getDataSegments(QList<QCPDataRange>& selectedSegments, QList<QCPDataRange>& unselectedSegments) const;
	[[nodiscard]] bool errorBarVisible(int index) const;
	[[nodiscard]] bool rectIntersectsLine(const QRectF& pixelRect, const QLineF& line) const;
...
public:
	...
	// getters:
	[[nodiscard]] QPen pen() const { return mPen; }
	[[nodiscard]] QPen selectedPen() const { return mSelectedPen; }
...
protected:
	...
	// non-virtual methods:
	[[nodiscard]] QLineF getRectClippedStraightLine(const QCPVector2D& point1, const QCPVector2D& vec, const QRect& rect) const;
	[[nodiscard]] QPen mainPen() const;
};
...
public:
	...
	[[nodiscard]] QPen pen() const { return mPen; }
	[[nodiscard]] QPen selectedPen() const { return mSelectedPen; }
	[[nodiscard]] QCPLineEnding head() const { return mHead; }
	[[nodiscard]] QCPLineEnding tail() const { return mTail; }
...
protected:
	...
	// non-virtual methods:
	[[nodiscard]] QLineF getRectClippedLine(const QCPVector2D& start, const QCPVector2D& end, const QRect& rect) const;
	[[nodiscard]] QPen mainPen() const;
};
...
protected:
	...
	// reimplemented virtual methods:
	void draw(QCPPainter* painter) Q_DECL_OVERRIDE;
	[[nodiscard]] QPointF anchorPixelPosition(int anchorId) const Q_DECL_OVERRIDE;

	// non-virtual methods:
	[[nodiscard]] QPen mainPen() const;
	[[nodiscard]] QBrush mainBrush() const;
};
...
public:
	...
	[[nodiscard]] QColor selectedColor() const { return mSelectedColor; }
	[[nodiscard]] QPen pen() const { return mPen; }
	[[nodiscard]] QPen selectedPen() const { return mSelectedPen; }
	[[nodiscard]] QBrush brush() const { return mBrush; }
	[[nodiscard]] QBrush selectedBrush() const { return mSelectedBrush; }
	...
	[[nodiscard]] QFont selectedFont() const { return mSelectedFont; }
	[[nodiscard]] QString text() const { return mText; }
	[[nodiscard]] Qt::Alignment positionAlignment() const { return mPositionAlignment; }
	[[nodiscard]] Qt::Alignment textAlignment() const { return mTextAlignment; }
	[[nodiscard]] double rotation() const { return mRotation; }
	[[nodiscard]] QMargins padding() const { return mPadding; }
...
protected:
	...
	// non-virtual methods:
	[[nodiscard]] QPointF getTextDrawPoint(const QPointF& pos, const QRectF& rect, Qt::Alignment positionAlignment) const;
	[[nodiscard]] QFont mainFont() const;
	[[nodiscard]] QColor mainColor() const;
	[[nodiscard]] QPen mainPen() const;
	[[nodiscard]] QBrush mainBrush() const;
};
...
public:
	...
	// getters:
	[[nodiscard]] QPixmap pixmap() const { return mPixmap; }
	[[nodiscard]] bool scaled() const { return mScaled; }
	[[nodiscard]] Qt::AspectRatioMode aspectRatioMode() const { return mAspectRatioMode; }
	[[nodiscard]] Qt::TransformationMode transformationMode() const { return mTransformationMode; }
	[[nodiscard]] QPen pen() const { return mPen; }
	[[nodiscard]] QPen selectedPen() const { return mSelectedPen; }
...
public:
	...
	*/
	...
	[[nodiscard]] QBrush selectedBrush() const { return mSelectedBrush; }
	[[nodiscard]] double size() const { return mSize; }
	[[nodiscard]] TracerStyle style() const { return mStyle; }
	[[nodiscard]] QCPGraph* graph() const { return mGraph; }
	[[nodiscard]] double graphKey() const { return mGraphKey; }
	[[nodiscard]] bool interpolating() const { return mInterpolating; }
...
public:
	...
	*/
	...
	[[nodiscard]] QPen selectedPen() const { return mSelectedPen; }
	[[nodiscard]] double length() const { return mLength; }
	[[nodiscard]] BracketStyle style() const { return mStyle; }
